...
export async function ProfileAnswerHistory({ type, scope, unified = false }: Props) {
  ...
  const answers = await getProfileAnswerHistory(user.id, type).catch(() => []);
  ...
  return <section className="legacy-history-page">
    ...
    {unified && <nav className="account-content-tabs history-type-tabs" aria-label="Yorum türü"><Link className={type === "TANIDIK" ? "active" : ""} href="/hesabim/yorumlarim?tur=tanidik">Tanıdık yorumları</Link><Link className={type === "COMMUNITY" ? "active" : ""} href="/hesabim/yorumlarim?tur=topluluk">Toplum yorumları</Link></nav>}
    <div className="legacy-history-list">
      {visible.map(({ answer, question }) => <article className="legacy-history-card" key={answer.id}>
        ...
        <p><strong>{answer.authorName || name}:</strong> {answer.body}</p>
