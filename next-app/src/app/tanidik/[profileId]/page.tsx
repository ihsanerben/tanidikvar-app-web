...
export default async function PublicProfilePage({ params, searchParams }: Props) {
  ...
  return <article className={`legacy-public-profile profile-role-${(profile.educationStatus??"user").toLowerCase()}`}>
    ...
    <section className="legacy-profile-contributions"><h2>Katkılar</h2><nav className="legacy-contribution-tabs" aria-label="Yorum türü"><Link scroll={false} className={selectedType === "TANIDIK" ? "active" : ""} aria-current={selectedType === "TANIDIK" ? "page" : undefined} href={`/tanidik/${profile.id}?yorum=tanidik`}>Tanıdık yorumları <span>{profile.tanidikAnswerCount}</span></Link><Link scroll={false} className={selectedType === "COMMUNITY" ? "active" : ""} aria-current={selectedType === "COMMUNITY" ? "page" : undefined} href={`/tanidik/${profile.id}?yorum=topluluk`}>Topluluk yorumları <span>{profile.communityAnswerCount}</span></Link></nav>{visibleAnswers.length ? <ol className="legacy-profile-answer-list">{visibleAnswers.map(answer => <li key={answer.id}><header><span>♥ {answer.likeCount}</span></header><div className="legacy-profile-answer-author"><span className={`legacy-avatar role-${(profile.educationStatus??"user").toLowerCase()} is-tanidik`}><span>{initials}</span><i>★★★</i></span><strong>{profile.name}</strong></div><p>{answer.body}</p><footer><time dateTime={answer.publishedAt}>{new Intl.DateTimeFormat("tr-TR", { dateStyle: "medium", timeZone: "Europe/Istanbul" }).format(new Date(answer.publishedAt))}</time><Link href={`/soru/${questionSegment(answer.questionTitle??"soru",answer.questionId)}`}>Soru detayı</Link></footer></li>)}</ol> : <div className="empty-state"><h3>Bu bölümde henüz yorum yok</h3><p>Yeni katkılar burada görünecek.</p></div>}</section>
