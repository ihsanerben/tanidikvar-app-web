...
const scopeText=(question:QuestionItem)=>question.scope==="GENERAL"?"Genel":[question.universityName,question.departmentName].filter(Boolean).join(" · ");
...
export default async function QuestionPage({params}:Props){
 ...
 return <article className="legacy-question-detail"><QuestionViewRecorder questionId={question.id}/><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(jsonLd).replace(/</g,"\\u003c")}}/><Link className="legacy-back-link" href="/sorular">← Sorulara dön</Link><div className="legacy-question-context"><span className={`legacy-scope-badge scope-${question.scope.toLowerCase()}${question.scope==="GENERAL"?" scope-dot-only":""}`}>{scopeText(question)}</span>{question.tags.filter(tag=>tag.available).map(tag=><span className="legacy-question-tag" key={tag.id}>#{tag.name}</span>)}</div><h1>{question.title}</h1>{question.body&&<p className="legacy-question-body">{question.body}</p>}
