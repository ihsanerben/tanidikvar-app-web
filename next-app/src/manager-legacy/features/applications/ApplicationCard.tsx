...
export function ApplicationCard({application:a,manager,reload}:{application:Application;manager:boolean;reload:()=>void}){
 ...
 {a.status==='APPROVED'&&!a.activeVerification&&<p className="application-reason" role="status">Tanıdık yetkin kaldırıldı.</p>}
 ...
 {manager&&!action&&<div className="application-actions">{a.status==='PENDING'&&<><button className="button" disabled={pending} onClick={()=>setAction('APPROVED')}>Kabul et</button><button className="button button-secondary" disabled={pending} onClick={()=>setAction('REJECTED')}>Reddet</button></>}{a.activeVerification&&<button className="button button-secondary" disabled={pending} onClick={()=>setAction('REVOKE')}>Tanıdık yetkisini kaldır</button>}</div>}
 {action&&<form onSubmit={e=>{e.preventDefault();void confirm()}}><p>{action==='APPROVED'?'Yukarıdaki eğitim ve profil bilgilerini kontrol ettin mi? Onay, Tanıdık yetkisi verir.':action==='REVOKE'?'Tanıdık yetkisi kaldırılacak ve bekleyen başvuru gerekçesiyle reddedilecek.':'Ret gerekçesi başvuru sahibine gösterilecek.'}</p>
