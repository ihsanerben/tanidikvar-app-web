...
import { Link, Navigate } from '@/manager-legacy/navigation'
...
function ApplicationsList({manager,canApply}:{manager:boolean;canApply:boolean}){
 const [data,setData]=useState<Applications|null>(null),[error,setError]=useState<ApiError|null>(null),[page,setPage]=useState(0),[status,setStatus]=useState(manager?'PENDING':''),[revision,setRevision]=useState(0),[,setSaved]=useState(false)
 ...
 const pending=data?.items.some(a=>a.status==='PENDING'), approved=data?.items.some(a=>a.activeVerification)
 return <section className="profile-page"><div className="profile-heading"><div>{manager&&<span className="eyebrow">MANAGER PANEL</span>}<h1>{manager?'Tanıdık başvuruları':'Başvurularım'}</h1></div></div>
 ...
 {error?<div className="auth-card"><AuthFormError error={error}/><button className="button" onClick={reload}>Tekrar dene</button></div>:!data?<p role="status">Başvurular yükleniyor…</p>:<><div className="application-list">{data.items.map(a=><ApplicationCard key={a.id+'-'+a.version+'-'+a.activeVerification} application={a} manager={manager} reload={reload}/>)}</div>
 {data.totalElements>data.size&&<nav className="application-actions" aria-label="Başvuru sayfaları"><button className="button button-secondary" disabled={page===0} onClick={()=>{setPage(page-1);setData(null)}}>Önceki</button><span>Sayfa {page+1} · {data.totalElements} başvuru</span><button className="button button-secondary" disabled={(page+1)*data.size>=data.totalElements} onClick={()=>{setPage(page+1);setData(null)}}>Sonraki</button></nav>}
