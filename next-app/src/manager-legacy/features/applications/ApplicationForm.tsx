...
import { Link } from '@/manager-legacy/navigation'
...
export function ApplicationForm({onSaved}:{onSaved:()=>void}){
 ...
 if(!eligible)return <div className="auth-card"><p>Tanıdık başvurusu için eğitim bilgilerini tamamla.</p><Link to="/profile">Profilime git</Link></div>
 ...
 <h2>Tanıdık başvurusu</h2><p>{profile.firstName} {profile.lastName}{profile.education&&` · ${profile.education.universityName} · ${profile.education.departmentName}`}</p>
