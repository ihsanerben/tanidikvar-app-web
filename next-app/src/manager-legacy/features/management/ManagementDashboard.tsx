...
export function ManagementDashboard(){
 ...
 const routes={activeUsers:'/manager/users?status=VISIBLE',disabledUsers:'/manager/users?status=HIDDEN',activeAdmins:'/manager/users?status=VISIBLE&authority=TANIDIK',pendingApplications:'/manager/applications',activeQuestions:'/manager/content?kind=QUESTION&status=VISIBLE',archivedQuestions:'/manager/content?kind=QUESTION&status=ALL',hiddenQuestions:'/manager/content?kind=QUESTION&status=HIDDEN',communityAnswers:'/manager/content?kind=COMMUNITY&status=ALL',adminAnswers:'/manager/content?kind=TANIDIK&status=ALL',likes:'/manager/analytics',views:'/manager/analytics'} as const
