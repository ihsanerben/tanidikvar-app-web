...
const messages: Record<string, string> = {
  ...
  UNKNOWN_UNIVERSITY: 'Üniversiteyi önce kataloğa veya toplu listeye ekle.',
  UNKNOWN_DEPARTMENT: 'Bölümü önce kataloğa veya toplu listeye ekle.',
  ANSWER_MODERATED: 'Bu yorum Manager tarafından gizlendi; düzenlenemez veya geri yüklenemez.',
  ADMIN_REQUIRED: 'Güncel Tanıdık yetkisi ve doğrulaması gerekiyor.',
...
async function raw(path: string, method: 'GET' | 'POST' | 'PUT' | 'DELETE', body?: unknown, signal?: AbortSignal, csrf?: string, binary=false): Promise<unknown> {
  ...
  try {
    response = await fetch(`${baseUrl}${path.replace(/^\/api\//, '/api/backend/').replaceAll('admin-applications', 'tanidik-applications').replaceAll('revoke-admin', 'revoke-tanidik')}`, {
  ...
  if (!response.ok) {
    ...
    if (isRecord(data) && isRecord(data.fieldErrors)) {
      const fieldMessages: Record<string,string> = {
        ...
        title: 'Soru başlığı 10–200 karakter olmalı.', body: path.includes('answers')?'Yorum 10–5000 karakter olmalı.':'Açıklama en fazla 5000 karakter olabilir.', scope: 'Kapsama uygun üniversite ve bölüm seç.', tagIds: 'En fazla 5 farklı tag seç.',
