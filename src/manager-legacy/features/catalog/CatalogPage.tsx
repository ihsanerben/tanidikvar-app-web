import { Link, Navigate, useSearchParams } from '@/manager-legacy/navigation'
...
export function CatalogPage({admin=false,tags=false}:{admin?:boolean;tags?:boolean}){
  ...
  if(auth.user.role!=='MANAGER' && !(admin && auth.user.role==='TANIDIK'))return <section className="status-page"><h1>Bu sayfaya erişim iznin yok.</h1><Link to="/account">Hesabıma dön</Link></section>
  ...
  return <section className="management-page"><span className="eyebrow">{admin?'TANIDIK':'MANAGER'} PANEL</span><h1>{admin?'Yeni konular ekle.':tags?'Tagler':'Üniversiteler ve Bölümler'}</h1>
