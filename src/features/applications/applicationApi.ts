...
export function parseApplication(v:unknown):Application {
 if(!isRecord(v)||!['id','applicantId','firstName','lastName','educationStatus','universityName','departmentName','submittedAt'].every(k=>typeof v[k]==='string')||!['PENDING','APPROVED','REJECTED'].includes(String(v.status))||typeof v.version!=='number'||typeof v.activeVerification!=='boolean'||!['occupation','company','documentFileId','reviewedBy','reviewedAt','rejectionReason'].every(k=>v[k]===null||typeof v[k]==='string')||!(v.graduationYear===null||typeof v.graduationYear==='number'))throw new ApiError(200,'INVALID_RESPONSE','Başvuru bilgileri alınamadı.')
...
export async function listApplications(manager:boolean,page:number,status:string,signal?:AbortSignal):Promise<Applications>{
 ...
 return {items:v.items.map(parseApplication),page:v.page,size:v.size,totalElements:v.totalElements}
}
export async function submitApplication(requestId:string,profileVersion:number,document:File|null){
 const data=new FormData();data.append('request',new Blob([JSON.stringify({requestId,profileVersion})],{type:'application/json'}));if(document)data.append('document',document)
 return parseApplication(await apiMutation('/api/me/admin-applications','POST',data))
}
export async function decide(a:Application,status:'APPROVED'|'REJECTED',reason:string){return parseApplication(await apiMutation(`/api/manager/admin-applications/${a.id}/decision`,'PUT',{status,reason,version:a.version}))}
