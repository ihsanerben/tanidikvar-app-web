...
function ManagerAccountForm({initial,reload}:{initial:ManagerAccount;reload:()=>void}){
 const auth=useAuth(),[account,setAccount]=useState(initial),[error,setError]=useState<ApiError|null>(null),[pending,setPending]=useState(false),[,setSaved]=useState(false),busy=useRef(false)
