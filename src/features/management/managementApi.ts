...
export interface ManagedUser {id:string;email:string;name:string|null;authority:string;educationStatus:string|null;universityName:string|null;departmentName:string|null;emailVerified:boolean;createdAt:string;lastLoginAt:string|null;deletedAt:string|null;version:number}
...
export function managedUser(v:unknown):ManagedUser{if(!isRecord(v)||!strings(v,['id','email','authority','createdAt'])||!strings(v,['name','educationStatus','universityName','departmentName','lastLoginAt','deletedAt'],true)||typeof v.emailVerified!=='boolean'||!Number.isSafeInteger(v.version))throw invalid();return v as unknown as ManagedUser}
