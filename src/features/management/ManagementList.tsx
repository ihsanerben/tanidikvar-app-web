...
export function ManagementList({users}:{users:boolean}){
 ...
 function filter(e:FormEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget),p=new URLSearchParams();for(const key of ['q','status','kind','authority','educationStatus']){const v=f.get(key);if(typeof v==='string'&&v)p.set(key,v)}navigate(p)}
 ...
 {data.items.length===0?<p>Eşleşen kayıt bulunamadı.</p>:users?<div className="managed-user-grid">{(data.items as ManagedUser[]).map(a=><Link className="managed-user-card" key={a.id+'-'+a.version} to={'/manager/users/'+a.id}><h2>{a.name??'Profil tamamlanmamış'}</h2><p>{a.email}</p><p className="managed-user-education">{a.universityName??'Üniversite bilgisi yok'}{a.departmentName&&` · ${a.departmentName}`}</p><dl><div><dt>Yetki</dt><dd>{a.authority==='MEMBER'?'Üye':a.authority}</dd></div><div><dt>Rol</dt><dd>{roleLabels[a.educationStatus??'USER']??'Profil tamamlanmamış'}</dd></div></dl></Link>)}</div>:(data.items as ManagedContent[]).map(a=><article className="question-card manager-content-card" key={a.id+'-'+a.version}><h2>{a.title}</h2><p>{a.authorName} · {a.moderatedAt?'Manager tarafından gizlendi':a.deletedAt?'Sahibi kaldırdı':'Görünür'}{a.archivedAt?' · Soru arşivlenmiş':''}</p><p className="manager-question-metrics">{questionDate(a.createdAt)}{a.kind==='QUESTION'&&<> · {a.viewCount} görüntülenme · {a.likeCount} beğeni · {a.communityAnswerCount+a.adminAnswerCount} yorum</>}</p>{a.questionHidden&&<p>Bağlı soru gizlenmiş; yorum herkese kapalı.</p>}<p className="answer-body">{a.body}</p><Link className="button button-info manager-content-detail" to={'/manager/questions/'+a.questionId}>Detay</Link></article>)}
