...
export function ManagementList({users}:{users:boolean}){
 ...
 function filter(e:FormEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget),p=new URLSearchParams();for(const key of ['q','status','kind']){const v=f.get(key);if(typeof v==='string'&&v)p.set(key,v)}navigate(p)}
 ...
 {data.items.length===0?<p>Eşleşen kayıt bulunamadı.</p>:data.items.map(a=><article className="question-card" key={a.id+'-'+a.version}>{'email' in a?<><h2>{a.name??'Profil tamamlanmamış'}</h2><p>{a.email}</p><p>{a.authority} · {a.educationStatus??'USER'} · {a.deletedAt?'Pasif':'Aktif'} · {a.emailVerified?'E-posta doğrulanmış':'E-posta doğrulanmamış'}</p>
 {a.authority==='MANAGER'?<p>Manager hesabı panelden pasifleştirilemez.</p>:<ManagementDecision label={a.deletedAt?'Hesabı geri yükle':'Hesabı pasifleştir'} explanation={a.deletedAt?'Kullanıcı yeniden giriş yapabilir. Eski oturumlar ve Admin yetkisi geri açılmaz.':'Tüm oturumlar kapanır. Admin yetkisi kaldırılır ve bekleyen başvurular reddedilir. İçeriklerde yazar Katılımcı olarak görünür.'} apply={reason=>userStatus(a,reason)} reload={reload}/>}</>:<><h2>{a.title}</h2><p>{a.authorName} · {a.moderatedAt?'Manager tarafından gizlendi':a.deletedAt?'Sahibi kaldırdı':'Görünür'}{a.archivedAt?' · Soru arşivlenmiş':''}</p>{a.questionHidden&&<p>Bağlı soru gizlenmiş; cevap herkese kapalı.</p>}<p className="answer-body">{a.body}</p>{!a.questionHidden&&(a.kind!=='QUESTION'||!a.moderatedAt)&&<p><Link to={'/questions/'+a.questionId}>Soruyu aç</Link></p>}
