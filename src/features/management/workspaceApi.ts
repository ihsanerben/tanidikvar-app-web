...
export interface UserDetail {user:ManagedUser;universityName:string|null;departmentName:string|null;graduationYear:number|null;avatarFileId:string|null;biography:string|null;occupation:string|null;company:string|null;linkedinUrl:string|null;portfolioUrl:string|null;verificationId:string|null;questions:number;communityAnswers:number;adminAnswers:number}
...
export async function getUserDetail(id:string,signal?:AbortSignal):Promise<UserDetail>{const v=await apiGet(`/api/manager/users/${id}`,signal);if(!isRecord(v)||!['universityName','departmentName','avatarFileId','biography','occupation','company','linkedinUrl','portfolioUrl','verificationId'].every(k=>nullable(v[k]))||!(v.graduationYear===null||Number.isSafeInteger(v.graduationYear))||!['questions','communityAnswers','adminAnswers'].every(k=>Number.isSafeInteger(v[k])))throw invalid();return {...v,user:managedUser(v.user)} as unknown as UserDetail}
