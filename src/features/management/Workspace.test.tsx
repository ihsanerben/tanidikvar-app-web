...
const user={id:'member',email:'member@example.test',name:'Ada Yılmaz',authority:'ADMIN',educationStatus:'UNIVERSITE_OGRENCISI',universityName:'Üniversite',departmentName:'Bölüm',emailVerified:true,createdAt:'2026-09-05T10:00:00Z',lastLoginAt:'2026-09-06T10:00:00Z',deletedAt:null,version:3}
...
it('question review shows hidden question and both answer types without recording a view',async()=>{
 const fetch=vi.fn(async(url:string)=>url.includes('/manager/questions/')?json({question:content,classification,answers:list([{...content,id:'community',kind:'COMMUNITY',body:'Topluluk katkısı',questionHidden:true},{...content,id:'admin',kind:'ADMIN',body:'Doğrulanmış katkı',questionHidden:true}])}):json(list([])));vi.stubGlobal('fetch',fetch);show('/questions/question');await screen.findByText(content.body);expect(screen.getByText('Topluluk katkısı')).toBeVisible();expect(screen.getByText('Doğrulanmış katkı')).toBeVisible();expect(screen.getAllByText('Soruyu soran')).toHaveLength(1);expect(screen.getAllByText('Yorumu yazan')).toHaveLength(2);expect(fetch.mock.calls.some(([url])=>url.includes('/views'))).toBe(false);expect(screen.queryByRole('button',{name:'Beğen'})).not.toBeInTheDocument()
})
it('question editing sends text and retains the form on a stale version',async()=>{
 const visible={...content,deletedAt:null,moderatedAt:null};const fetch=vi.fn(async(url:string,o:RequestInit)=>url.endsWith('/csrf')?json({token:'csrf'}):o.method==='PUT'?json({code:'STALE_VERSION'},409):url.includes('/manager/questions/')?json({question:visible,classification,answers:list([])}):json(list([])));vi.stubGlobal('fetch',fetch);show('/manager/questions/question');fireEvent.click(await screen.findByLabelText('İçerik işlemleri'));fireEvent.click(screen.getByRole('button',{name:'Düzenle'}));fireEvent.change(await screen.findByLabelText('Düzeltme gerekçesi'),{target:{value:'Sınıflandırma incelendi'}});fireEvent.click(screen.getByRole('button',{name:'Soruyu kaydet'}));await screen.findByRole('button',{name:'Güncel soruyu yükle'});expect(screen.getByLabelText('Düzeltme gerekçesi')).toHaveValue('Sınıflandırma incelendi');const body=JSON.parse(fetch.mock.calls.find(([,o])=>o.method==='PUT')![1].body as string);expect(body).toEqual({...classification,education:undefined,title:content.title,body:content.body,reason:'Sınıflandırma incelendi'});expect(screen.getByLabelText('Soru başlığı')).toHaveValue(content.title);expect(screen.getByLabelText('Soru açıklaması')).toHaveValue(content.body)
})
it('user detail revokes only the current verification with a required reason',async()=>{
 const detail={user,firstName:'Ada',lastName:'Yılmaz',universityName:'Üniversite',departmentName:'Bölüm',graduationYear:null,avatarFileId:null,biography:null,occupation:null,company:null,linkedinUrl:null,portfolioUrl:null,verificationId:'verification',questions:2,communityAnswers:3,adminAnswers:4}
...
it('edits user details in a dialog and closes it after save',async()=>{
 const detail={user,firstName:'Ada',lastName:'Yılmaz',universityName:'Üniversite',departmentName:'Bölüm',graduationYear:null,avatarFileId:null,biography:null,occupation:null,company:null,linkedinUrl:null,portfolioUrl:null,verificationId:'verification',questions:2,communityAnswers:3,adminAnswers:4}
 let reads=0
 const fetch=vi.fn(async(url:string,o:RequestInit)=>url.endsWith('/csrf')?json({token:'csrf'}):o.method==='PUT'?json({...detail,firstName:'Ayşe',user:{...user,name:'Ayşe Yılmaz',version:4}}):url.includes('/manager/users/member')?(reads++,json(reads>1?{...detail,firstName:'Ayşe',user:{...user,name:'Ayşe Yılmaz',version:4}}:detail)):json(list([])))
 vi.stubGlobal('fetch',fetch);show('/manager/users/member')
 expect(await screen.findByRole('button',{name:'Kullanıcı bilgilerini düzenle'})).toBeVisible()
 expect(screen.queryByLabelText('Ad')).not.toBeInTheDocument()
 fireEvent.click(screen.getByRole('button',{name:'Kullanıcı bilgilerini düzenle'}))
