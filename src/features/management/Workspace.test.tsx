...
const application={id:'application',applicantId:'member',firstName:'Ada',lastName:'Yılmaz',educationStatus:'UNIVERSITE_OGRENCISI',universityName:'Üniversite',departmentName:'Bölüm',graduationYear:null,occupation:null,company:null,documentFileId:'document',status:'PENDING',submittedAt:'2026-09-05T10:00:00Z',reviewedBy:null,reviewedAt:null,rejectionReason:null,version:0,activeVerification:false}
...
it('application detail includes document preview and history with Blob cleanup',async()=>{
 const create=vi.fn(()=> 'blob:test-document'),revoke=vi.fn();vi.stubGlobal('URL',class extends URL {static createObjectURL=create;static revokeObjectURL=revoke});vi.stubGlobal('fetch',vi.fn(async(url:string)=>url.includes('/download')?({ok:true,status:200,headers:new Headers(),blob:async()=>new Blob(['%PDF-test'])} as Response):url.includes('/users/')?json(list([application])):json(application)));const v=show('/manager/applications/application');await screen.findByTitle('Başvuru belgesi');expect(screen.getByRole('heading',{name:'Başvuru geçmişi'})).toBeVisible();expect(screen.getByTitle('Başvuru belgesi')).not.toHaveAttribute('sandbox');v.unmount();expect(revoke).toHaveBeenCalledWith('blob:test-document')
})
