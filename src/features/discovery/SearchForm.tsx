...
export function SearchForm({value,onSearch,label='Soru ara',filterButton}:{value:string;onSearch:(q:string)=>void;label?:string;filterButton?:ReactNode}) {
 ...
 <div><input key={value} aria-label={label} placeholder={label} name="q" type="search" maxLength={100} defaultValue={value}/>{filterButton}<button className="button" type="submit">Ara</button></div></form>
}
