...
it('searches admins and opens their public profile with summary counts',async()=>{
 ...
 fireEvent.change(screen.getByLabelText('Admin adı ara'),{target:{value:'cagri'}});fireEvent.click(screen.getByRole('button',{name:'Ara'}));await waitFor(()=>expect(fetch.mock.calls.some(([url])=>new URL(url).pathname==='/api/admins'&&new URL(url).searchParams.get('q')==='cagri')).toBe(true))
