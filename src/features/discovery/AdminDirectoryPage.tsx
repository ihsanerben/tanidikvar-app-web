...
export function AdminDirectoryPage(){
 ...
 <SearchForm label="Admin adı ara" value={params.get('q')??''} onSearch={q=>filter('q',q)} filterButton={<button type="button" className={`button button-secondary filter-toggle${filtersOpen?' is-active':''}`} aria-expanded={filtersOpen} aria-controls="admin-filters" onClick={()=>setFiltersOpen(v=>!v)}>Filtrele{params.get('status')==='ACTIVE'?' · 1':''}</button>}/>
 {filtersOpen&&<div id="admin-filters" className="question-filters"><div className="auth-card admin-directory-filters"><label htmlFor="admin-status">Admin durumu</label><select id="admin-status" value={params.get('status')??''} onChange={e=>filter('status',e.target.value)}><option value="">Tüm Adminler</option><option value="ACTIVE">Aktif Adminler</option></select><button className="filter-clear-button" type="button" onClick={()=>setParams({})}>Filtreleri temizle</button></div></div>}
...
function AdminResults({query,activeOnly,retry,page}:{query:string;activeOnly:boolean;retry:()=>void;page:(value:number)=>void}){
 ...
 <ProfileAvatar fileId={a.avatarFileId} name={a.name} className="admin-photo" isAdmin={a.activeAdmin}/><div><h2>{a.name}</h2><p className="admin-directory-education">{a.universityName||'Üniversite bilgisi yok'}{a.departmentName&&` · ${a.departmentName}`}</p><span className="admin-directory-role">Admin</span><p className="admin-directory-counts"><strong>{a.answerCount}</strong> Admin cevabı <span>·</span> <strong>{a.communityAnswerCount}</strong> topluluk yorumu</p></div></Link>)}</div>
