...
export function ProfileTrigger({id,name,avatarFileId,isAdmin=false}:{id:string|null;name:string;avatarFileId?:string|null;isAdmin?:boolean}){
 ...
 return <><button ref={trigger} type="button" className="profile-trigger" aria-label={name+' profilini görüntüle'} onClick={()=>setOpen(true)}><ProfileAvatar fileId={avatarFileId??null} name={name} className="answer-avatar" isAdmin={isAdmin}/>{name}</button>{open&&<PublicProfilePopup key={id} id={id} close={close}/>}</>
}
function PublicProfilePopup({id,close}:{id:string;close:()=>void}){
 ...
 {error?<><h2 id="public-profile-title">Profil</h2><AuthFormError error={error}/><button onClick={()=>{setError(null);reload(r=>r+1)}}>Tekrar dene</button></>:!profile?<><h2 id="public-profile-title">Profil</h2><p role="status">Profil yükleniyor…</p></>:<><div className="public-profile-popup-header"><ProfileAvatar fileId={profile.avatarFileId} name={profile.name} isAdmin={profile.role==='ADMIN'}/><div><h2 id="public-profile-title">{profile.name}</h2><p className="profile-role">{roleLabels[profile.educationStatus]??profile.educationStatus}</p></div></div><dl className="account-summary">{profile.universityName&&<div><dt>Üniversite</dt><dd>{profile.universityName}</dd></div>}{profile.departmentName&&<div><dt>Bölüm</dt><dd>{profile.departmentName}</dd></div>}{profile.graduationYear&&<div><dt>Mezuniyet yılı</dt><dd>{profile.graduationYear}</dd></div>}{profile.occupation&&<div><dt>Meslek</dt><dd>{profile.occupation}</dd></div>}{profile.company&&<div><dt>Şirket</dt><dd>{profile.company}</dd></div>}</dl>{profile.biography&&<p className="profile-biography">{profile.biography}</p>}<ProfileLinks linkedinUrl={profile.linkedinUrl} portfolioUrl={profile.portfolioUrl} profileId={profile.id}/></>}
 </dialog>
}
