...
function PublicProfilePopup({id,close,detailHref}:{id:string;close:()=>void;detailHref?:string}){
 ...
 {error?<><h2 id="public-profile-title">Profil</h2><AuthFormError error={error}/><button onClick={()=>{setError(null);reload(r=>r+1)}}>Tekrar dene</button></>:!profile?<><h2 id="public-profile-title">Profil</h2><p role="status">Profil yükleniyor…</p></>:<><div className="public-profile-popup-header"><ProfileAvatar fileId={profile.avatarFileId} name={profile.name} isAdmin={profile.role==='ADMIN'} educationStatus={profile.educationStatus}/><div><h2 id="public-profile-title">{profile.name}</h2><p className="profile-role">{roleLabels[profile.educationStatus]??profile.educationStatus}</p></div></div><dl className="account-summary">{profile.universityName&&<div><dt>Üniversite</dt><dd>{profile.universityName}</dd></div>}{profile.departmentName&&<div><dt>Bölüm</dt><dd>{profile.departmentName}</dd></div>}{profile.graduationYear&&<div><dt>Mezuniyet yılı</dt><dd>{profile.graduationYear}</dd></div>}{profile.occupation&&<div><dt>Meslek</dt><dd>{profile.occupation}</dd></div>}{profile.company&&<div><dt>Şirket</dt><dd>{profile.company}</dd></div>}</dl>{profile.biography&&<p className="profile-biography">{profile.biography}</p>}<ProfileLinks linkedinUrl={profile.linkedinUrl} portfolioUrl={profile.portfolioUrl}/>{detailHref&&<a className="button button-secondary" href={detailHref}>{detailHref.startsWith('/manager/')?'Yönetim detayını aç':'Profil detayını aç'}</a>}</>}
 </dialog>
}
