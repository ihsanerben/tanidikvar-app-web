...
function PublicProfileLoader({id}:{id:string}){
 ...
 return <section className="profile-page public-profile-page"><Link className="back-link" to="/questions">← Sorulara dön</Link><div className="profile-heading public-profile-hero"><div className={`public-profile-avatar ${profile.role==='ADMIN'?'is-admin':''}`}><ProfileAvatar fileId={profile.avatarFileId} name={profile.name} decorated={false}/>{profile.role==='ADMIN'&&<span className="public-profile-admin-stars" aria-label="Admin"><AdminStars educationStatus={profile.educationStatus}/></span>}</div><div className="public-profile-info"><h1>{profile.name}</h1><p>{profile.universityName}{profile.departmentName&&` · ${profile.departmentName}`}</p><p className="public-profile-role">{roleLabels[profile.educationStatus]??profile.educationStatus}{profile.role==='ADMIN'&&<span>Admin</span>}</p>{hasDetails&&<dl className="public-profile-details">{profile.graduationYear&&<div><dt>Mezuniyet yılı</dt><dd>{profile.graduationYear}</dd></div>}{profile.occupation&&<div><dt>Meslek</dt><dd>{profile.occupation}</dd></div>}{profile.company&&<div><dt>Şirket</dt><dd>{profile.company}</dd></div>}<div><dt>Hesap açılışı</dt><dd><time dateTime={profile.createdAt}>{membershipDate(profile.createdAt)}</time></dd></div></dl>}{profile.biography&&<p className="profile-biography">{profile.biography}</p>}</div><ProfileLinks linkedinUrl={profile.linkedinUrl} portfolioUrl={profile.portfolioUrl}/></div><ProfileCommentSection title="Admin yorumları" data={admin} type="admin" onPage={setAdminPage}/><ProfileCommentSection title="Topluluk yorumları" data={community} type="community" onPage={setCommunityPage}/></section>
}
