...
import { ProfileLinks } from './PublicProfilePopup'
...
function PublicProfileLoader({id}:{id:string}){
 ...
 const hasDetails=Boolean(profile.graduationYear||profile.occupation||profile.company||profile.createdAt)
 return <section className="profile-page public-profile-page"><Link className="back-link" to="/questions">← Sorulara dön</Link><div className="profile-heading public-profile-hero"><div className={`public-profile-avatar ${profile.role==='ADMIN'?'is-admin':''}`}><ProfileAvatar fileId={profile.avatarFileId} name={profile.name} decorated={false} educationStatus={profile.educationStatus}/>{profile.role==='ADMIN'&&<span className="public-profile-admin-stars" aria-label="Admin"><AdminStars educationStatus={profile.educationStatus}/></span>}</div><div className="public-profile-info"><h1>{profile.name}</h1><p>{profile.universityName}{profile.departmentName&&` · ${profile.departmentName}`}</p><p className="public-profile-role">{roleLabels[profile.educationStatus]??profile.educationStatus}{profile.role==='ADMIN'&&<span>Admin</span>}</p>{hasDetails&&<dl className="public-profile-details">{profile.graduationYear&&<div><dt>Mezuniyet yılı</dt><dd>{profile.graduationYear}</dd></div>}{profile.occupation&&<div><dt>Meslek</dt><dd>{profile.occupation}</dd></div>}{profile.company&&<div><dt>Şirket</dt><dd>{profile.company}</dd></div>}<div><dt>Hesap açılışı</dt><dd><time dateTime={profile.createdAt}>{membershipDate(profile.createdAt)}</time></dd></div></dl>}{profile.biography&&<p className="profile-biography">{profile.biography}</p>}</div><ProfileLinks linkedinUrl={profile.linkedinUrl} portfolioUrl={profile.portfolioUrl}/></div><ProfileCommentSection title="Admin yorumları" data={admin} type="admin" onPage={setAdminPage}/><ProfileCommentSection title="Topluluk yorumları" data={community} type="community" onPage={setCommunityPage}/></section>
}

function ProfileCommentSection({title,data,type,onPage}:{title:string;data:Page<Answer>|Page<AdminAnswer>;type:'community'|'admin';onPage:(page:number)=>void}){const {items,page,size,totalElements}=data;return <section className="profile-comments"><h2>{title}</h2>{items.length===0?<p>Henüz görünür yorum yok.</p>:items.map(item=><article className="answer-card profile-comment-card" key={item.id}><span className={`comment-kind ${type}`}>{type==='admin'?'Admin yorumu':'Topluluk yorumu'}</span><p className="answer-body">{item.body}</p><div className="profile-comment-footer"><time dateTime={item.publishedAt}>{questionDate(item.publishedAt)}</time><Link className="button button-secondary" to={`/questions/${item.questionId}?tab=${type}#yorum-${item.id}`}>Soru detayı</Link></div></article>)}{totalElements>size&&<nav className="pagination" aria-label={`${title} sayfaları`}><button type="button" disabled={page===0} onClick={()=>onPage(page-1)}>Önceki</button><span>Sayfa {page+1}</span><button type="button" disabled={(page+1)*size>=totalElements} onClick={()=>onPage(page+1)}>Sonraki</button></nav>}</section>}
