...
function parse(value:unknown):Profile {
  if(!isRecord(value) || typeof value.completed!=='boolean' || typeof value.version!=='number'
    ...
    || !['linkedinUrl','portfolioUrl'].every(key=>value[key]==null || typeof value[key]==='string')
  ...
  return {firstName:value.firstName as string|null,lastName:value.lastName as string|null,educationStatus:value.educationStatus as EducationStatus|null,
    ...
    biography:value.biography as string|null,occupation:value.occupation as string|null,company:value.company as string|null,linkedinUrl:value.linkedinUrl as string|null??null,portfolioUrl:value.portfolioUrl as string|null??null,completed:value.completed,version:value.version}
}
