...
function AdminAnswerPage({path,page,showQuestion,setPage,retry,excludeId}:{excludeId:string|null;path:string;page:number;showQuestion:boolean;setPage:(n:number)=>void;retry:()=>void}){
 ...
 return <><p className="answer-count">{data.totalElements} Admin cevabı</p>{data.items.length?data.items.filter(a=>a.id!==excludeId).map(a=><AdminAnswerCard key={a.id} answer={a} showQuestion={showQuestion}/>):<p>Henüz Admin cevabı yok.</p>}
