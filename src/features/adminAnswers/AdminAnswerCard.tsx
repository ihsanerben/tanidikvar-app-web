...
export function AdminAnswerCard({answer:a,showQuestion=false}:{answer:AdminAnswer;showQuestion?:boolean}){
 ...
 <p className="answer-body">{a.body}</p><div className="question-meta"><time dateTime={a.publishedAt}>{questionDate(a.publishedAt)}</time>{a.editedAt&&<span>Düzenlendi · <time dateTime={a.editedAt}>{questionDate(a.editedAt)}</time></span>}{a.deletedAt&&<span>Kaldırılmış cevap · yalnız sen görüyorsun</span>}</div></article>
}
