...
function Panel({id}:{id:string}){
 ...
 <Link to="/my-answers">Topluluk yorumlarım</Link><Link to="/profile">Profilimi düzenle</Link>{quota?.activeAdmin&&<Link to={'/admins/'+id}>Herkese açık profilim</Link>}</nav>
 <div className="history-scope-filter"><label>Kapsam<select value={scope} onChange={e=>{const p=new URLSearchParams();if(e.target.value)p.set('scope',e.target.value);setParams(p)}}><option value="">Tümü</option><option value="GENERAL">Genel</option><option value="UNIVERSITY">Üniversite</option><option value="UNIVERSITY_DEPARTMENT">Üniversite ve bölüm</option></select></label></div><AdminAnswerFeed key={scope} path={`/api/me/admin-answers${scope?`?scope=${scope}`:''}`} showQuestion/><Link className="button button-secondary account-back-button" to="/account">Hesabıma dön</Link></section>
}
