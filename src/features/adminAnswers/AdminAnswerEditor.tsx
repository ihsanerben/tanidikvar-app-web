...
export function AdminAnswerEditor({questionId,initial,onSaved,onCancel,reload}:{questionId:string;initial?:AdminAnswer;onSaved:()=>void;onCancel:()=>void;reload:()=>void}){
 ...
 <div className="answer-actions"><button className="button" disabled={pending}>{pending?'Kaydediliyor…':initial?'Admin cevap değişikliklerini kaydet':'Admin cevabını yayınla'}</button><button type="button" disabled={pending} onClick={onCancel}>Vazgeç</button></div></form>
}
