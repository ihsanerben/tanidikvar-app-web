...
export function AdminAnswerEditor({questionId,initial,onSaved,onCancel,reload}:{questionId:string;initial?:AdminAnswer;onSaved:()=>void;onCancel:()=>void;reload:()=>void}){
 ...
 <label htmlFor="admin-answer-body">{initial?'Admin yorumunı düzenle':'Admin yorumun'}</label>
 ...
 <div className="answer-actions"><button className="button" disabled={pending}>{pending?'Kaydediliyor…':initial?'Admin yorum değişikliklerini kaydet':'Admin yorumunı yayınla'}</button><button type="button" disabled={pending} onClick={onCancel}>Vazgeç</button></div></form>
}
