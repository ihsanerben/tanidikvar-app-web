...
export function AdminAnswerEditor({questionId,initial,onSaved,onCancel,reload}:{questionId:string;initial?:AdminAnswer;onSaved:()=>void;onCancel:()=>void;reload:()=>void}){
 ...
 <label htmlFor="admin-answer-body">{initial?'Admin cevabını düzenle':'Admin cevabın'}</label><p id="admin-answer-help" className="field-help">10–5.000 karakter. Düzenleme yeni soru hakkı tüketmez.</p>
 <textarea id="admin-answer-body" required minLength={10} maxLength={5000} rows={6} disabled={pending} value={body} onChange={e=>setBody(e.target.value)} aria-invalid={!!error?.fieldErrors.body} aria-describedby="admin-answer-help"/>
