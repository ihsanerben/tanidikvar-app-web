...
export function OwnAdminAnswer({questionId,archived,reload,onLoaded}:{questionId:string;archived:boolean;reload:()=>void;onLoaded?:(id:string|null)=>void}){
 ...
 {!a&&active&&!archived&&<div className="answer-compose-action"><button className="answer-add-button" disabled={data.quota.remaining===0} onClick={()=>setEditing(true)} aria-label="Admin yorumu yaz"><span aria-hidden="true">+</span> Yorum yaz</button></div>}
