...
export function OwnAdminAnswer({questionId,archived,reload,onLoaded}:{questionId:string;archived:boolean;reload:()=>void;onLoaded?:(id:string|null)=>void}){
 ...
 {!a&&active&&!archived&&<div className="answer-compose-action"><button className="answer-add-button" disabled={data.quota.remaining===0} onClick={()=>setEditing(true)} aria-label="Admin cevabı yaz"><span aria-hidden="true">+</span> Cevap yaz</button></div>}
 {!a&&active&&data.quota.remaining===0&&<p>Bugünkü beş cevap hakkını kullandın.</p>}
 {a?.moderatedAt&&<p>Admin cevabın Manager tarafından gizlendi.</p>}{a&&<div className="own-answer-row"><AdminAnswerCard answer={a}/><div className="answer-actions">
 ...
 {editing&&<ComposerDialog title={a?'Admin cevabını düzenle':'Admin cevabı'} onClose={()=>setEditing(false)}><p>Kalan cevap hakkın: {data.quota.remaining} / {data.quota.limit}</p><AdminAnswerEditor questionId={questionId} initial={a??undefined} onSaved={reload} reload={reload} onCancel={()=>setEditing(false)}/></ComposerDialog>}
