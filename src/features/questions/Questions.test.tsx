...
it('keeps form and request identity after unknown network result, then navigates after retry',async()=>{
  ...
  fireEvent.change(screen.getByLabelText('Soru açıklaması'),{target:{value:'Sorunun ayrıntılı açıklaması'}})
  ...
  expect(JSON.parse(writes[0][1].body as string)).toMatchObject({content:{body:'Sorunun ayrıntılı açıklaması'}})
...
it('keeps the existing question description when only its title changes',async()=>{
  ...
  expect(await screen.findByLabelText('Soru başlığı')).toHaveValue(q.title)
...
it('defaults to admin answers and switches the visible answer type',async()=>{
 ...
 expect(screen.getByRole('tabpanel',{name:'Admin yorumları (0)'})).toBeVisible()
 fireEvent.click(screen.getByRole('tab',{name:'Topluluk yorumları (0)'}))
 expect(screen.getByRole('tabpanel',{name:'Topluluk yorumları (0)'})).toBeVisible()
})
