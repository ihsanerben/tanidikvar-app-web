...
function QuestionDetail({question:q,reload,opening,initialTab,hash}:{question:Question;reload:()=>void;opening:Opening;initialTab:string;hash:string}) {
  ...
  return <article className="question-detail"><Link className="back-link" to="/questions">← Sorulara dön</Link><QuestionContext question={q}/><h1>{q.title}</h1>
    <div className="question-meta question-byline"><div className="question-byline-author"><ProfileTrigger id={q.authorId} name={q.authorName} detailHref={q.authorId?`/profiles/${q.authorId}`:undefined}/><time dateTime={q.createdAt}>{questionDate(q.createdAt)}</time>{q.editedAt && <span>Düzenlendi · <time dateTime={q.editedAt}>{questionDate(q.editedAt)}</time></span>}</div><QuestionEngagement questionId={q.id} initial={q.statistics} archived={!!q.archivedAt} opening={opening} answersRevision={answersRevision} onStatistics={setStatistics}>{auth.user&&auth.user.id!==q.authorId&&!reported&&<button type="button" className="question-report-trigger" aria-label="Soruyu şikâyet et" title="Soruyu şikâyet et" aria-expanded={reporting} onClick={()=>setReporting(v=>!v)}>!</button>}</QuestionEngagement></div>
