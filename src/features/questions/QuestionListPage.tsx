...
export function QuestionListPage({mine=false}:{mine?:boolean}) {
  const auth=useAuth()
  return mine?<QuestionGate profile={false}><QuestionList key={auth.user?.id} mine/></QuestionGate>:<QuestionList key="public"/>
}
function QuestionList({mine=false}:{mine?:boolean}) {
  ...
  useEffect(()=>{
    ...
    listQuestions(`${mine?'/api/me/questions':'/api/questions'}?${query}`,controller.signal).then(data=>{if(!controller.signal.aborted){setResult(data);setLoadedQuery(query);setError(null);setLoading(false)}})
      .catch(e=>{if(!controller.signal.aborted){setError(formError(e));setLoading(false)}})
    return ()=>controller.abort()
  },[mine,query,revision,auth.user?.id])
  function filter(key:string,value:string|null){const next=new URLSearchParams(params);next.delete('page');if(value)next.set(key,value);else next.delete(key);setLoading(true);setParams(next)}
  function page(value:number){const next=new URLSearchParams(params);next.set('page',String(value));setLoading(true);setParams(next)}
  return <section className="questions-page"><div className="questions-heading"><div><span className="eyebrow">{mine?'BENİM PAYLAŞIMLARIM':'MERAK ETTİKLERİN BURADA'}</span><h1>{mine?'Sorularım':'Sorular'}</h1><p>{mine?'Aktif ve arşivlediğin sorularını buradan bulabilirsin.':'Üniversiteyi ve bölümü, merak edilen sorulardan tanımaya başla.'}</p></div><Link className="button" to="/questions/new">Soru sor</Link></div>
    <div className="question-navigation"><Link to={mine?'/questions':'/my-questions'}>{mine?'Tüm sorular':'Sorularım'}</Link></div>
    {!mine && <details className="question-filters"><summary>Soruları filtrele</summary><div className="auth-card">
      ...
      <button type="button" onClick={()=>{setUniversity(null);setEducation(null);setTag(null);setLoading(true);setParams({});setRevision(r=>r+1)}}>Filtreleri temizle</button>
    </div></details>}
    {error?<div className="auth-card"><AuthFormError error={error}/><button onClick={()=>{setLoading(true);setRevision(r=>r+1)}}>Tekrar dene</button></div>:waiting?<p role="status">Sorular yükleniyor…</p>:result?.items.length===0?<div className="question-empty"><h2>Henüz soru yok.</h2><p>{mine?'İlk sorunu paylaşarak başlayabilirsin.':'Bu seçimlere uygun soru bulunamadı. Filtreleri değiştirebilir veya yeni bir soru sorabilirsin.'}</p></div>:<div className="question-list">{result?.items.map(q=><QuestionCard key={q.id} question={q}/>)}</div>}
