...
export function QuestionCard({question:q}:{question:Question}) {
  return <article className="question-card"><QuestionContext question={q}/><h2><Link to={`/questions/${q.id}`}>{q.title}</Link></h2>
    ...
    <div className="question-meta question-byline"><div className="question-byline-author"><span>{q.authorName}</span><time dateTime={q.createdAt}>{questionDate(q.createdAt)}</time>{q.archivedAt && <span>Arşivlendi</span>}</div><QuestionStats statistics={q.statistics}/></div>
  </article>
}
