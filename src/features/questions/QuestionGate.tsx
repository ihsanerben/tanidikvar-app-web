...
export function QuestionGate({children,profile=true}:{children:ReactNode;profile?:boolean}) {
  ...
  if(!auth.user)return <section className="status-page"><h1>Önce giriş yap.</h1><Link className="button" to="/login">Giriş yap</Link></section>
  if(auth.user.role==='MANAGER')return <section className="status-page"><h1>Yönetim hesabı</h1><Link to="/manager">Yönetim paneline dön</Link></section>
