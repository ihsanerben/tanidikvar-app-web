...
function Account({status}:{status:boolean}){
 ...
 {user.role!=='ADMIN'&&user.role!=='MANAGER'&&<Link className="button button-secondary" to="/my-questions">Sorularım</Link>}
 {user.role==='MANAGER'&&<Link className="button button-secondary" to="/manager">Manager Panel</Link>}
 {user.role==='ADMIN'&&<Link className="button button-secondary" to="/admin">Admin cevaplarım</Link>}
