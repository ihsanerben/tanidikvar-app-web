...
function payload(text:string,reason:string){let value:unknown;try{value=JSON.parse(text)}catch{throw new ApiError(400,'INVALID_IMPORT','JSON biçimi geçersiz. Tırnakları, virgülleri ve parantezleri örnekle karşılaştır.')}if(!isRecord(value)||!Array.isArray(value.universities)||!Array.isArray(value.departments)||!Array.isArray(value.matches))throw new ApiError(400,'INVALID_IMPORT','JSON; universities, departments ve matches dizilerini içermeli.');return {...value,reason}}
