...
it('shows the question and removed comment with a working detail link',async()=>{
 ...
 render(<MemoryRouter><MyAnswersPage/></MemoryRouter>);expect(await screen.findByText('Kendi deneyimim burada.')).toBeVisible();expect(screen.getByText('Kaldırıldı')).toBeVisible();expect(screen.getByRole('link',{name:'Soru detayı'})).toHaveAttribute('href','/questions/q?tab=community#yorum-a')
})
