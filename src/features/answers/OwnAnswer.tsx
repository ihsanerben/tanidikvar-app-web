...
export function OwnAnswer({questionId,archived,onChange,reloadQuestion}:{questionId:string;archived:boolean;onChange:()=>void;reloadQuestion:()=>void}) {
  ...
  return <div className="own-answer"><h3>Cevabım</h3>
    ...
    {own.deletedAt?<><p>Cevabını kaldırdın. Diğer kullanıcılar bu cevabı göremez.</p><p className="answer-body removed-answer">{own.body}</p>{archived?<p>Soru arşivlendiği için cevabını geri yükleyemezsin.</p>:<button className="button" disabled={pending} onClick={()=>void status(false)}>Cevabı geri yükle</button>}</>:
      editing && !archived?<AnswerEditor key={own.version} questionId={questionId} initial={own} onSaved={saved} onCancel={()=>setEditing(false)} reload={reload} reloadQuestion={reloadQuestion}/>:
      <><p>{archived?'Soru arşivlenmiş; cevabını yalnız kaldırabilirsin.':'Bu soruya cevap verdin. Eklemek istediklerin varsa mevcut cevabını düzenleyebilirsin.'}</p><div className="answer-actions">{!archived && <button className="button" disabled={pending} onClick={()=>{setEditing(true);setNotice('')}}>Cevabımı düzenle</button>}<button disabled={pending} onClick={()=>setConfirm(true)}>Cevabımı kaldır</button></div></>}
