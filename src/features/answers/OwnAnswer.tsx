...
export function OwnAnswer({questionId,archived,onChange,reloadQuestion,onLoaded}:{questionId:string;archived:boolean;onChange:()=>void;reloadQuestion:()=>void;onLoaded?:(id:string|null)=>void}) {
 ...
 {own&&<div className="own-answer-row"><article className="answer-card"><h3><ProfileTrigger id={own.authorId} name={own.authorName} avatarFileId={own.avatarFileId} detailHref={own.authorId?`/profiles/${own.authorId}`:undefined}/></h3><p className="answer-body">{own.body}</p>
