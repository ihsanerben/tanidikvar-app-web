...
export function AnswerList({questionId,revision,excludeId=null}:{questionId:string;revision:number;excludeId?:string|null}) {
  ...
  return <div className="community-list">
    {data?.items.filter(a=>a.id!==excludeId).map(a=><article className="answer-card" id={`yorum-${a.id}`} key={a.id}><div className="answer-author"><div><h3><ProfileTrigger id={a.authorId} name={a.authorName} avatarFileId={a.avatarFileId} detailHref={a.authorId?`/profiles/${a.authorId}`:undefined}/></h3>{a.educationStatus&&<p className="author-role">{roleLabels[a.educationStatus]??a.educationStatus}</p>}<div className="question-meta"><time dateTime={a.publishedAt}>{questionDate(a.publishedAt)}</time>{a.editedAt && <span>Yorum düzenlendi · <time dateTime={a.editedAt}>{questionDate(a.editedAt)}</time></span>}</div></div></div><p className="answer-body">{a.body}</p><AnswerLikeButton answerId={a.id} initialCount={a.likeCount}/></article>)}
