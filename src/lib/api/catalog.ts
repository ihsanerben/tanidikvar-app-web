...
export type CatalogOverview = { universityCount:number; programCount:number; optionCount:number; statisticsCount:number; rankedOptionCount:number; institutionTypes:LabelCount[]; cities:LabelCount[]; degreeLevels:LabelCount[]; scoreTypes:LabelCount[]; yearly:YearCatalogStatistics[]; topUndergraduatePrograms:LabelCount[];topAssociatePrograms:LabelCount[];universitiesByPlaced:LabelCount[];universitiesByProgramCount:LabelCount[];rankBands:LabelCount[];lastSynchronizedAt:string|null };
