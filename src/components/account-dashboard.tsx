...
type Resolved=Item&{label:string;detail:string;href:string;kind?:string;badge?:string;question?:QuestionItem};
type University={id:string;name:string};type Person={id:string;name:string;educationVerified?:boolean;universityName?:string|null;departmentName?:string|null;educationStatus?:string|null};

async function resolveItem(item:Item):Promise<Resolved>{
  try {
    if(item.targetType==="QUESTION"){const value=await apiRequest<QuestionItem>(`/questions/${item.targetId}`);return{...item,label:value.title,detail:"Soru",question:value,href:`/soru/${questionSegment(value.title,value.id)}`};}
...
export function AccountDashboard({mode="all"}:{mode?:"all"|"saved"|"follows"|"notifications"}){
  ...
  const savedList=saved===null?<p>Yükleniyor…</p>:saved.length?<ul className="legacy-question-list">{saved.map(item=>item.question?<QuestionCard key={item.id} question={item.question}/>:<li key={item.id} className="saved-question-unavailable">İçerik artık yayında olmayabilir.</li>)}</ul>:<p>Henüz kaydettiğin bir soru yok.</p>;
