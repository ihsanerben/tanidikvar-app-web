...
export function ManagerShell({children}:{children:ReactNode}){
 ...
 return <div className="manager-shell"><a className="skip-link" href="#manager-main">İçeriğe geç</a><header className="manager-header"><button ref={toggle} className="manager-menu-toggle" aria-expanded={open} aria-controls="manager-sidebar" onClick={()=>setOpen(value=>!value)}>Menü</button><Link href="/yonetim" className="manager-brand" aria-label="TanıdıkVar Yönetim"><Image src="/logo-wordmark.svg" alt="" width={110} height={28} /><span>Yönetim</span></Link><Link href="/yonetim/hesabim">Yönetim hesabım</Link></header><aside id="manager-sidebar" className={`manager-sidebar${open?" is-open":""}`}><p>YÖNETİM PANELİ</p><nav aria-label="Yönetim menüsü">{links.map(([href,label])=><Link key={href} aria-current={pathname===href||(href!=="/yonetim"&&pathname.startsWith(href+"/"))?"page":undefined} className={pathname===href||(href!=="/yonetim"&&pathname.startsWith(href+"/"))?"active":""} href={href} onClick={()=>setOpen(false)}>{label}</Link>)}</nav></aside><main id="manager-main" className="manager-main">{children}</main></div>;
}
