...
export function ModalShell({open,onClose,title,children,className=""}:{open:boolean;onClose:()=>void;title:string;children:ReactNode;className?:string}){
 ...
 if(!open)return null;
 return <div className="ui-modal-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)onClose();}}><section className={`ui-modal ${className}`.trim()} role="dialog" aria-modal="true" aria-labelledby={titleId}><button className="ui-modal-close" type="button" onClick={onClose} aria-label="Pencereyi kapat"><span aria-hidden="true">×</span></button><h2 id={titleId}>{title}</h2>{children}</section></div>;
}
