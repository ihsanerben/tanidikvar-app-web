...
type Profile={firstName:string;lastName:string;educationStatus:"YKS_ADAYI"|"UNIVERSITE_OGRENCISI"|"MEZUN";programId:string|null;education:{universityId:string;departmentId:string}|null;classYear:number|null;graduationYear:number|null;biography:string|null;occupation:string|null;company:string|null;linkedinUrl:string|null;portfolioUrl:string|null;version:number};
...
export function ProfileForm(){
 const [profile,setProfile]=useState<Profile|null>(null),[universities,setUniversities]=useState<Item[]>([]),[educations,setEducations]=useState<(Education&{displayName:string})[]>([]),[universityId,setUniversityId]=useState(""),[educationStatus,setEducationStatus]=useState<Profile["educationStatus"]>("YKS_ADAYI"),[message,setMessage]=useState("");
 useEffect(()=>{void Promise.all([apiRequest<Profile>("/me/profile"),apiRequestAllPages<Item>("/universities")]).then(([p,u])=>{setProfile(p);setUniversities(u.toSorted((a,b)=>turkish.compare(a.name,b.name)));setUniversityId(p.education?.universityId??"");setEducationStatus(p.educationStatus);}).catch(()=>setMessage("Profil yüklenemedi."));},[]);
 useEffect(()=>{if(!universityId){setEducations([]);return;}apiRequestAllPages<Education>(`/catalog-programs?universityId=${universityId}`).then(items=>setEducations(uniquePrograms(items))).catch(()=>setMessage("Programlar yüklenemedi."));},[universityId]);
 if(!profile)return <p className="muted">{message||"Profil yükleniyor…"}</p>;
 const current=profile;
 async function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();setMessage("");const data=new FormData(event.currentTarget);try{const status=data.get("educationStatus");const updated=await apiRequest<Profile>("/me/profile",{method:"PUT",body:JSON.stringify({firstName:data.get("firstName"),lastName:data.get("lastName"),educationStatus:status,universityId:status==="YKS_ADAYI"?null:universityId,programId:status==="YKS_ADAYI"?null:data.get("programId"),departmentId:null,classYear:status==="UNIVERSITE_OGRENCISI"?Number(data.get("classYear")):null,graduationYear:status==="MEZUN"?Number(data.get("graduationYear")):null,biography:data.get("biography")||null,occupation:data.get("occupation")||null,company:data.get("company")||null,linkedinUrl:data.get("linkedinUrl")||null,portfolioUrl:data.get("portfolioUrl")||null,version:current.version})});setProfile(updated);setMessage("Profil güncellendi.");}catch(reason){setMessage(reason instanceof Error?reason.message:"Profil güncellenemedi.");}}
