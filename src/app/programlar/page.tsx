...
export default async function ProgramsPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 const filters:ProgramFilters={query:(one(params.q)??"").trim().slice(0,100),programName:(one(params.programName)??"").trim().slice(0,100),universityName:(one(params.universityName)??"").trim().slice(0,100),city:(one(params.city)??"").slice(0,100),institutionType:one(params.tur)??"",degreeLevel:one(params.duzey)??"",scoreType:one(params.puan)??"",rankFrom:one(params.siraMin)?Number(one(params.siraMin)):undefined,rankTo:one(params.siraMax)?Number(one(params.siraMax)):undefined,scoreFrom:one(params.puanMin)?Number(one(params.puanMin)):undefined,scoreTo:one(params.puanMax)?Number(one(params.puanMax)):undefined,year:Number(one(params.yil)??2025),sort:"RANK",page:0,size:24};
