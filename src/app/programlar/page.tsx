...
export default async function ProgramsPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 const requestedSort=one(params.sirala),sort:ProgramFilters["sort"]=["RANK","NAME","SCORE","QUOTA"].includes(requestedSort??"")?requestedSort as ProgramFilters["sort"]:"RANK";
 const filters:ProgramFilters={query:(one(params.q)??"").trim().slice(0,100),programName:(one(params.programName)??"").trim().slice(0,100),universityName:(one(params.universityName)??"").trim().slice(0,100),city:(one(params.city)??"").slice(0,100),institutionType:one(params.tur)??"",degreeLevel:one(params.duzey)??"",scoreType:one(params.puan)??"",rankFrom:one(params.siraMin)?Number(one(params.siraMin)):undefined,rankTo:one(params.siraMax)?Number(one(params.siraMax)):undefined,scoreFrom:one(params.puanMin)?Number(one(params.puanMin)):undefined,scoreTo:one(params.puanMax)?Number(one(params.puanMax)):undefined,year:Number(one(params.yil)??2026),sort,page:0,size:24};
 ...
 return <main className="content-section programs-page"><PageTitle help="Programları ad, üniversite, şehir, kurum, düzey ve puan türüne göre filtreleyebilirsin. Ayrıntılı başarı sırası ve puan aralıkları üç nokta altında bulunur.">Programlar</PageTitle>
  <form className="program-filter discovery-filter" action="/programlar" role="search">
   ...
   <label className="sort-field">Sırala<select name="sirala" defaultValue={sort}><option value="RANK">Başarı sırasına göre</option><option value="NAME">Ada göre</option><option value="SCORE">Taban puana göre</option><option value="QUOTA">Kontenjana göre</option></select></label>
