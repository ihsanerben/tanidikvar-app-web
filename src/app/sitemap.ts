...
import { getCatalogPrograms, getUniversities, getUniversityDepartments } from "@/lib/api/catalog";
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    const [questions, people, firstPrograms] = await Promise.all([allQuestions(), allPeople(), getCatalogPrograms({size:100})]);
