...
test('heart is in the stats row; answer tabs preserve position and composers are dialogs',async({page})=>{
 ...
 const stats=page.getByRole('region',{name:'Soru etkileşimleri'}),like=stats.getByRole('button',{name:'Beğen',exact:true})
 await expect(like).toBeVisible();await like.click();await expect(stats.getByRole('button',{name:'Beğeniyi geri al'})).toHaveAttribute('aria-pressed','true')
 await stats.getByRole('button',{name:'Beğeniyi geri al'}).click();await expect(like).toHaveAttribute('aria-pressed','false')
 await expect(page.getByRole('tab',{name:/Admin yorumları/})).toHaveAttribute('aria-selected','true')
 await page.getByRole('button',{name:'Admin yorumu yaz'}).click();await expect(page.getByRole('dialog')).toBeVisible();await page.getByRole('button',{name:'Vazgeç'}).click()
 await page.getByRole('tab',{name:/Topluluk yorumları/}).scrollIntoViewIfNeeded();await page.waitForTimeout(400)
 const y=await page.evaluate(()=>scrollY)
 await page.getByRole('tab',{name:/Topluluk yorumları/}).click();await page.getByRole('button',{name:'Topluluk yorumu yap'}).waitFor();await page.waitForTimeout(400)
 expect(Math.abs(await page.evaluate(()=>scrollY)-y)).toBeLessThan(3)
 await page.getByRole('button',{name:'Topluluk yorumu yap'}).click();await expect(page.getByRole('dialog')).toBeVisible()
...
test('admin keeps application history and education role; photo editor opens from heading',async({page})=>{
 ...
 await page.goto('/applications');await expect(page.getByRole('heading',{name:'Başvurularım'})).toBeVisible();await expect(page.getByLabel('e-Devlet öğrenci / mezun belgesi')).toHaveCount(0)
