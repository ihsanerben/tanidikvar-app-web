...
import { account,promoteTestManager,uploadPhoto } from './accounts'
...
test('candidate profile can be completed, edited and reopened',async({page,request},testInfo)=>{
  ...
  await uploadPhoto(page);await page.getByRole('button',{name:'Profili kaydet'}).click()
  await expect(page).toHaveURL(/\/account$/);await page.goto('/profile')
  ...
  await page.getByRole('button',{name:'Profili kaydet'}).click();await expect(page).toHaveURL(/\/account$/);await page.goto('/profile')
  expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true)
  await page.screenshot({path:testInfo.outputPath('profile.png'),fullPage:true})
  await page.goto('/account');await expect(page.locator('.account-contact')).toContainText('Deniz Yılmaz')
...
test('manager identity and catalog linking stay independent from education',async({page,request},testInfo)=>{
  ...
  await page.getByLabel('Ad',{exact:true}).fill('Deniz');await page.getByLabel('Soyad',{exact:true}).fill('Yönetici');await page.getByRole('button',{name:'Bilgilerimi kaydet'}).click();await expect(page.getByText('Yönetim kimliğin kaydedildi.')).toBeVisible()
  ...
  async function add(name:string){await page.getByLabel('Yeni kayıt adı').fill(name);await page.getByLabel('Ekleme gerekçesi').fill('Yerel katalog incelemesi');await page.getByRole('button',{name:'Ekle',exact:true}).click();await expect(page.locator('.catalog-list li').filter({hasText:name})).toBeVisible()}
  ...
  await page.getByRole('button',{name:'Üniversite–bölüm eşleşmeleri'}).click();await page.getByLabel('Üniversite',{exact:true}).selectOption({label:university});await page.getByLabel('Eklenecek bölüm',{exact:true}).selectOption({label:department});await page.getByLabel('Eşleştirme gerekçesi').fill('Üniversitede bölüm var');await page.getByRole('button',{name:'Üniversiteye bölüm ekle'}).click();await expect(page.locator('.catalog-list li').filter({hasText:department})).toBeVisible()
  await page.goto('/manager/tags');await add(tag)
  await page.locator('.catalog-list li').filter({hasText:tag}).getByRole('button',{name:'Pasife al',exact:true}).click();await expect(page.getByText('0 bağlı profil · 0 bağlı soru')).toBeVisible();await page.getByRole('button',{name:'Kaydı pasifleştir',exact:true}).click();await page.getByLabel('İşlem gerekçesi').fill(`Tag incelemeye alındı ${suffix}`);await page.getByRole('button',{name:'Kaydı pasifleştir — onayla'}).click();await expect(page.locator('.catalog-list li').filter({hasText:tag}).getByText('Pasif',{exact:true})).toBeVisible()
  await page.locator('.catalog-list li').filter({hasText:tag}).getByRole('button',{name:'Aktife al',exact:true}).click();await page.getByRole('button',{name:'Kaydı geri aç',exact:true}).click();await page.getByLabel('İşlem gerekçesi').fill('İnceleme tamamlandı');await page.getByRole('button',{name:'Kaydı geri aç — onayla'}).click();await expect(page.locator('.catalog-list li').filter({hasText:tag}).getByText('Aktif',{exact:true})).toBeVisible()
