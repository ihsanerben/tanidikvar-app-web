...
test('questions support all scopes, filtering, editing, stale forms and readable archives',async({page,request,browser},info)=>{
  ...
  await page.goto(`/manager/content?kind=COMMUNITY&q=${suffix}`)
  await page.getByRole('button',{name:'İçeriği gizle',exact:true}).click()
  await page.getByLabel('İşlem gerekçesi').fill('Topluluk cevabı moderasyon testi.')
  await page.getByRole('button',{name:'İçeriği gizle — onayla'}).click()
  await expect(page.getByRole('button',{name:'İçeriği geri yükle',exact:true})).toBeVisible()
  expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true)
  await page.screenshot({path:info.outputPath('manager-content.png'),fullPage:true})
  ...
  await page.goto(`/manager/content?kind=COMMUNITY&q=${suffix}`)
  await page.getByRole('button',{name:'İçeriği geri yükle',exact:true}).click()
  await page.getByLabel('İşlem gerekçesi').fill('İnceleme tamamlandı.')
  await page.getByRole('button',{name:'İçeriği geri yükle — onayla'}).click()
  await expect(page.getByRole('button',{name:'İçeriği gizle',exact:true})).toBeVisible()
